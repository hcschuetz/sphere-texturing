...
/** perimeter/radius ratio of circles */
const TAU = 2 * Math.PI;
...
function uv12pos(
  u0: number, v_we: number, v_p: number,
  radius_p: number, y_we: number, y_p: number,
) {
  const u1 = u0 + .1, u2 = u0 + .2;
  return B.Matrix.GetAsMatrix3x3(
    ...
    .multiply(createMat3(
      radius   * Math.cos(u0 * TAU), y_we, radius   * Math.sin(u0 * TAU),
      radius_p * Math.cos(u1 * TAU), y_p , radius_p * Math.sin(u1 * TAU),
      radius   * Math.cos(u2 * TAU), y_we, radius   * Math.sin(u2 * TAU),
    ))
  );
}
...
mat3 uv12pos[4] = mat3[4](${
  [uv12pos_14, uv12pos_5, uv12pos_0, uv12pos_19]
  .map(mat => `
  mat3(${mat})`)
  .join(",")}
);
...
export const createIcoSprite = (
  ...
  Object.assign(
    new B.ProceduralTexture(name, {width, height: getHeight(width)}, "IcoSprite", scene)
...
const getHeight = (width: number) =>
  Math.round(width * Math.sqrt(3) / (5 * (1 - dv)));
